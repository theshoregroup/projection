import { Body, CodeInline, Container, Head, Heading, Html, Link, Preview, Row, Section, Text } from "react-email";
import z from "zod";
					...
					<Container className="bg-white rounded-xl px-10 py-6">
						<Row>
							...
							<Text className="text-base">
								Hey {user.name}, we need to verify your email to get started with projection.
							</Text>
						</Row>

						<Section>
							<Link href={url} className="rounded-lg bg-primary px-[18px] py-3 text-primary-foreground">
								Verify your email
							</Link>

							<Text className="text-xs text-muted-foreground">
								Or copy the token below and paste it into the verification screen:
							</Text>
							<CodeInline className="font-mono p-4 text-lg bg-accent">{token}</CodeInline>
						</Section>

						<Container className="mt-6">
							<Text className="text-muted-foreground text-sm">
								You're receiving this email because {user.name} registered {user.email} on projection.
							</Text>
						</Container>
					</Container>
